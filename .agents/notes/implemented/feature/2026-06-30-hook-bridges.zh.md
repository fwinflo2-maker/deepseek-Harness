...
**Stop 循环防护。** `dsh-hook-protocol` 导出 `createStopGuard(maxConsecutiveBlocks)`：按 agent、按轮次统计 `Stop` 钩子已强制继续的次数。每个桥接在运行该点之前用 `guard.active(agent.id, turn)` 标注 `stop_hook_active`（Claude Code 与 Codex 都定义了该标志），`deny` 结果只有在 `guard.block(agent.id, turn)` 允许时才引导继续；超过上限后桥接发出警告并让轮次关闭。上限是两个桥接上的 `maxConsecutiveStopBlocks` 配置字段，默认为 `DEFAULT_MAX_CONSECUTIVE_STOP_BLOCKS`（8，Claude Code 的覆盖点）。新轮次从零开始，`agent/disposed` 会丢弃记录。

**钩子 `continue:false`（硬停止）。** 合并出的 `stop` 在每个有 agent 处于作用域内的点都会被落实：桥接以 `stopReason`（或 `halted by <point> hook`）调用 `agent.cancel({ kind: 'hook', reason })`，因此轮次以 `hook` 原因结束为 `aborted`——取消原因 `AgentCancelCause` 早已为此保留了该变体。该点自身的决策仍然适用（PreToolUse 的停止同时拒绝工具，因为中止信号先生效），而没有 agent 的运行（轮次之外的直接工具执行）只在结果中记录停止，没有可取消的对象。
