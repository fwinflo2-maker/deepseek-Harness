...
**Stop loop-guard.** `dsh-hook-protocol` exports `createStopGuard(maxConsecutiveBlocks)`: a per-agent, per-turn count of the blocks a `Stop` hook has already forced. Each bridge stamps `stop_hook_active` from `guard.active(agent.id, turn)` before running the point (Claude Code and Codex both define the flag), and a `deny` outcome only steers when `guard.block(agent.id, turn)` admits it; past the cap the bridge warns and lets the turn close. The cap is the `maxConsecutiveStopBlocks` config field on both bridges, defaulting to `DEFAULT_MAX_CONSECUTIVE_STOP_BLOCKS` (8, Claude Code's override point). A new turn starts from zero, and `agent/disposed` drops the record.

**Hook `continue:false` (hard halt).** The merged `stop` is honored at every point that runs with an agent in scope: the bridge calls `agent.cancel({ kind: 'hook', reason })` with `stopReason` (or `halted by <point> hook`), so the turn ends `aborted` with a `hook` cause — the cancel cause `AgentCancelCause` already carried that variant for this purpose. The point's own decision still applies (a PreToolUse halt also denies the tool, since the aborted signal wins first), and a run without an agent (a direct tool execution outside a turn) records the halt in its outcome with nothing to cancel.
